...
var ErrMissingQueryParam = errors.New("query parameter is not set")
...
func (d *Dadata) FindAddress(query string) (*AddressSuggestionsResponse, error) {
	...
	client := &http.Client{}
	req, _ := http.NewRequest("POST", fmt.Sprintf("%s/suggest/address", API_URL), bytes.NewBuffer(body))
	...
	var addr AddressSuggestionsResponse
